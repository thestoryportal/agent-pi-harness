"""Pocket Pick MCP Server — SQLite-backed personal knowledge base."""
...
async def serve(sqlite_database: Path | None = None) -> None:
    """Start the Pocket Pick MCP server."""
    logger.info("Starting Pocket Pick MCP server")

    db_path = (
        sqlite_database
        if sqlite_database is not None
        else DEFAULT_SQLITE_DATABASE_PATH
    )
    ...
    async def list_tools() -> list[Tool]:
        return [
            Tool(
                ...
                inputSchema=PocketAdd.model_json_schema(),
            ),
            Tool(
                name=PocketTools.ADD_FILE,
                description=(
                    "Add a new item to your pocket pick database from a file"
                ),
                inputSchema=PocketAddFile.model_json_schema(),
            ),
            Tool(
                name=PocketTools.FIND,
                description=(
                    "Find items in your pocket pick database by text and tags"
                ),
                inputSchema=PocketFind.model_json_schema(),
            ),
            Tool(
                name=PocketTools.LIST,
                description=(
                    "List items in your pocket pick database, "
                    "optionally filtered by tags"
                ),
                inputSchema=PocketList.model_json_schema(),
            ),
            Tool(
                name=PocketTools.LIST_TAGS,
                description=(
                    "List all tags in your pocket pick database "
                    "with their counts"
                ),
                inputSchema=PocketListTags.model_json_schema(),
            ),
            Tool(
                name=PocketTools.REMOVE,
                description=(
                    "Remove an item from your pocket pick database by ID"
                ),
                inputSchema=PocketRemove.model_json_schema(),
            ),
            Tool(
                name=PocketTools.GET,
                description=(
                    "Get an item from your pocket pick database by ID"
                ),
                inputSchema=PocketGet.model_json_schema(),
            ),
            Tool(
                name=PocketTools.BACKUP,
                description=(
                    "Backup your pocket pick database to a specified location"
                ),
                inputSchema=PocketBackup.model_json_schema(),
            ),
            Tool(
                ...
                inputSchema=PocketToFileById.model_json_schema(),
            ),
        ]
    ...
    async def call_tool(name: str, arguments: dict) -> list[TextContent]:
        ...
        # Ensure the database exists for every command
        from .modules.init_db import init_db
        ...
        match name:
            case PocketTools.ADD:
                ...
                return [
                    TextContent(
                        type="text",
                        text=(
                            f"Added item with ID: {result.id}\n"
                            f"Text: {result.text}\n"
            ...
            case PocketTools.ADD_FILE:
                ...
                return [
                    TextContent(
                        type="text",
                        text=(
                            f"Added file content with ID: {result.id}\n"
                            f"From file: {arguments['file_path']}\n"
                            f"Tags: {', '.join(result.tags)}"
                        ),
                    )
                ]
            ...
            case PocketTools.LIST:
                ...
                if not results:
                    return [
                        TextContent(type="text", text="No items found.")
                    ]
            ...
            case PocketTools.LIST_TAGS:
                ...
                if not results:
                    return [
                        TextContent(type="text", text="No tags found.")
                    ]
            ...
            case PocketTools.REMOVE:
                command = RemoveCommand(
                    id=arguments["id"], db_path=db_path
                )
                ...
                if result:
                    return [
                        TextContent(
                            ...
                            text=(
                                f"ID: {result.id}\n"
                                f"Created: {result.created.isoformat()}\n"
                                f"Tags: {', '.join(result.tags)}\n"
                                f"Text: {result.text}"
                            ),
                        )
            ...
            case PocketTools.TO_FILE_BY_ID:
                command = ToFileByIdCommand(
                    ...
                    output_file_path_abs=Path(
                        arguments["output_file_path_abs"]
                    ),
                    db_path=db_path,
                )
                ...
                if result:
                    return [
                        TextContent(
                            type="text",
                            text=(
                                "Content written successfully to "
                                f"{command.output_file_path_abs}"
                            ),
                        )
                    ]
                else:
