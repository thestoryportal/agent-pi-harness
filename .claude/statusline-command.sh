#!/bin/sh
# ArhuGula status line — global Starship-style + harness info

# Catppuccin Mocha ANSI colors (256-colour approximations)
CYAN='\033[38;5;81m'
...
# Truncate to last 4 path segments
...
# Count hook files dynamically
hook_count=$(find "$cwd/.claude/hooks" -name '*.py' ! -name '_base.py' 2>/dev/null | wc -l | tr -d ' ')
...
# ArhuGula harness info
line="${line} $(printf "${DIMWHITE}hooks:%s fail-open${RESET}" "$hook_count")"
