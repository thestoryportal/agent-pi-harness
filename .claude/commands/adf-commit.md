---
description: Stage and commit ADF (agent-deployment-framework) changes from ArhuGula session. Autogenerates commit message from diff unless message provided as argument.
...
Show the commit message, then output this command for the user to run:

```
! git -C ~/Projects/agent-deployment-framework add <file1> <file2> ... && git -C ~/Projects/agent-deployment-framework commit -F /tmp/adf-commit.txt
```
