...
"""
Usage:
    uv run agents/sfa/sfa_codebase_context_agent.py \
...
def git_list_files(
    ...
    try:
        ...
        # Get all files tracked by git (use cwd= to avoid os.chdir race)
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                try:
                    ...
                    # If not valid JSON, default to not relevant (safe fallback)
                    result = {
...
def complete_task_output_relevant_files(reasoning: str) -> str:
    ...
    try:
        ...
        if not RELEVANT_FILES:
            console.log("[yellow]No relevant files to output[/yellow]")
